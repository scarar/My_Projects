...
def get_ctx_from_hash(hash):
    matched = re.search(r'^\$(argon2(i|d|id))\$v=([0-9]+)\$([tpm=0-9,]+)\$([a-zA-Z0-9/\+]+)\$([a-zA-Z0-9/\+]+)', hash)
    if matched:
...
def parseArgs():
    ...
    parser.add_argument("-w", "--wordlist", dest="wordlist", action="store", type=str, required=True, help="Wordlist")
